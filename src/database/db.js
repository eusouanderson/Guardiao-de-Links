...
const createDatabase = (dbPath) => {
  ...
  const readStudyState = () => {
    ...
    if (!row) {
      return { prompt: '', updatedAt: null, lesson: null, completionCount: 0 };
    }
  ...
  const enqueueStudy = (prompt) => {
    ...
    return db
      .prepare('INSERT INTO study_queue (prompt, created_at, sort_order) VALUES (?, ?, ?)')
      .run(prompt, new Date().toISOString(), nextSortOrder);
  };
  ...
  const listQueue = () =>
    db
      .prepare(
        'SELECT id, prompt, created_at AS createdAt FROM study_queue ORDER BY sort_order, id'
      )
      .all();
